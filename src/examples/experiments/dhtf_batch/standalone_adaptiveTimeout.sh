...
# ./src/examples/experiments/dhtf_batch/standalone_adaptiveTimeout.sh /src/examples/experiments/dhtf_batch dhtf_experiment.argos

if [ "$#" -ne 2 ]; then
    echo "Usage: standalone_adaptiveTimeout.sh (from src folder) <base_config_dir> <base_config_file_name>"
    exit 11
fi
...
res_dir=$wdir/"results_dhtf/standalone_newWalkParam/selfAdaptiveTimeout"
if [[ ! -e $res_dir ]]; then
...
param_dir=$res_dir/"NewTimeout_Regions_Adaptive_"$date_time"_robots#"$numrobots"_timeout#"$timeout"_respawn#"$reactivation_timer"_NumAreas#"$desired_num_of_areas"_redAreas#"$hard_tasks"_"$experiment_length"#seconds"
